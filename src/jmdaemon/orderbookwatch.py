...
class OrderbookWatch(object):
    def set_msgchan(self, msgchan):
        ...
        try:
            ...
            self.db.execute(
                "CREATE TABLE orderbook(counterparty TEXT, "
                "oid INTEGER, ordertype TEXT, minsize INTEGER, "
                "maxsize INTEGER, txfee INTEGER, cjfee TEXT);"
            )
            self.db.execute(
                "CREATE TABLE fidelitybonds(counterparty TEXT, "
    ...
    def on_order_seen(
        self, counterparty, oid, ordertype, minsize, maxsize, txfee, cjfee
    ):
        try:
            self.dblock.acquire(True)
            ...
            if int(minsize) < 0 or int(minsize) > 21 * 10**14:
                log.debug(
                    "Got invalid minsize: {} from {}".format(
            ...
            if int(minsize) < self.dust_threshold:
                ...
                log.debug(
                    "{} has dusty minsize, capping at {}".format(
            ...
            if int(maxsize) < 0 or int(maxsize) > 21 * 10**14:
                log.debug(
                    "Got invalid maxsize: " + maxsize + " from " + counterparty
                )
                return
            if int(txfee) < 0:
                log.debug(
                    "Got invalid txfee: {} from {}".format(txfee, counterparty)
                )
                return
            if int(minsize) > int(maxsize):
                fmt = (
                    "Got minsize bigger than maxsize: {} - {} from {}"
                ).format
            ...
            if ordertype in [
                ...
                try:
                    ...
                    log.debug(
                        "Got non integer coinjoin fee: "
                        + str(cjfee)
                        + " for an absoffer from "
                        + counterparty
                    )
                    return
            self.db.execute(
            ...
            )  # any parseable Decimal is a valid cjfee
        except InvalidOperation:
            log.debug(
                "Got invalid cjfee: " + str(cjfee) + " from " + counterparty
            )
        except Exception as e:
            log.debug(
                "Error parsing order " + str(oid) + " from " + counterparty
            )
    ...
    def on_fidelity_bond_seen(self, nick, bond_type, fidelity_bond_proof_msg):
        ...
        maker_nick = nick
        if not fidelity_bond_sanity_check.fidelity_bond_sanity_check(
            fidelity_bond_proof_msg
        ):
            log.debug(
                "Failed to verify fidelity bond for {}, skipping.".format(
                    maker_nick
                )
            )
            return
        try:
            self.dblock.acquire(True)
            self.db.execute(
                "DELETE FROM fidelitybonds WHERE counterparty=?;", (nick,)
            )
            self.db.execute(
    ...
    def on_nick_leave(self, nick):
        try:
            self.dblock.acquire(True)
            self.db.execute(
                'DELETE FROM orderbook WHERE counterparty=?;', (nick,)
            )
            self.db.execute(
                'DELETE FROM fidelitybonds WHERE counterparty=?;', (nick,)
            )
