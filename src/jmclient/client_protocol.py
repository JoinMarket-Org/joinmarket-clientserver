...
class BIP78ClientProtocol(BaseClientProtocol):
    ...
    def connectionMade(self):
        ...
        if self.mode == "sender":
            ...
            d = self.callRemote(
                commands.BIP78ReceiverInit, netconfig=netconfig
            )
    ...
    def on_BIP78_RECEIVER_ORIGINAL_PSBT(self, body, params):
        ...
        retval = self.success_callback(
            body.encode("utf-8"),
            bdict_sdict_convert(params, output_binary=True),
        )
        if not retval[0]:
            d = self.callRemote(
                commands.BIP78ReceiverSendError,
                errormsg=retval[1],
                errorcode=retval[2],
            )
        else:
            d = self.callRemote(
                commands.BIP78ReceiverSendProposal, psbt=retval[1]
            )
        self.defaultCallbacks(d)
        return {"accepted": True}
    ...
    def on_BIP78_SENDER_UP(self):
        d = self.callRemote(
            commands.BIP78SenderOriginalPSBT,
...
class SNICKERClientProtocol(BaseClientProtocol):
    ...
    def connectionMade(self):
        netconfig = {
            "socks5_host": jm_single().config.get(
                "PAYJOIN", "onion_socks5_host"
            ),
            "socks5_port": jm_single().config.get(
                "PAYJOIN", "onion_socks5_port"
            ),
        ...
        if isinstance(self.client, SNICKERReceiver):
            d = self.callRemote(
                commands.SNICKERReceiverInit, netconfig=netconfig
            )
        else:
            d = self.callRemote(
                commands.SNICKERProposerInit, netconfig=netconfig
            )
            self.defaultCallbacks(d)
    ...
    def on_SNICKER_PROPOSER_UP(self):
        ...
        for s in self.servers:
            if s == "":
                continue
            d = self.callRemote(commands.SNICKERRequestPowTarget, server=s)
            self.defaultCallbacks(d)
        return {"accepted": True}
    ...
    def on_SNICKER_RECEIVE_POW_TARGET(self, server, targetbits):
        ...
        d = self.callRemote(
            commands.SNICKERProposerPostProposals,
    ...
    def on_SNICKER_RECEIVER_UP(self):
        if self.oneshot:
            ...
            self.proposal_poll_loop = task.LoopingCall(self.poll_for_proposals)
            poll_interval = int(
                60.0
                * float(
                    jm_single().config.get(
                        "SNICKER", "polling_interval_minutes"
                    )
                )
            )
    ...
    def on_SNICKER_PROPOSALS_SERVER_RESPONSE(self, response, server):
        self.client.info_callback(
            "Response from server: " + str(server) + " was: " + str(response)
        )
...
class JMClientProtocol(BaseClientProtocol):
    def __init__(self, factory, client, nick_priv=None):
        ...
        if not nick_priv:
            self.nick_priv = hashlib.sha256(os.urandom(16)).digest() + b"\x01"
        else:
    ...
    def set_nick(self):
        ...
        self.nick_pkh_raw = hashlib.sha256(
            binascii.hexlify(self.nick_pubkey)
    ...
    def on_JM_INIT_PROTO(
        ...
        self.set_nick()
        d = self.callRemote(commands.JMStartMC, nick=self.nick)
        self.defaultCallbacks(d)
        return {'accepted': True}
    ...
    def on_JM_REQUEST_MSGSIG_VERIFY(
        ...
        if not btc.ecdsa_verify(str(msg), sig, pubkey_bin):
            ...
            if not btc.ecdsa_verify(
                str(msg[: -len(hostid)] + hostid.lower()), sig, pubkey_bin
            ):
                jlog.debug(
                    "nick signature verification failed, ignoring: "
                    + str(nick)
                )
                verif_result = False
        # check that nick matches hash of pubkey
        nick_pkh_raw = hashlib.sha256(pubkey.encode("ascii")).digest()[
            :hashlen
        ]
        ...
        if not nick_unpadded == btc.base58.encode(nick_pkh_raw):
            jlog.debug(
                "Nick hash check failed, expected: "
                + str(nick_unpadded)
                + ", got: "
    ...
    def make_tx(self, nick_list, tx):
        d = self.callRemote(commands.JMMakeTx, nick_list=nick_list, tx=tx)
        self.defaultCallbacks(d)
...
class JMMakerClientProtocol(JMClientProtocol):
    ...
    def clientStart(self):
        ...
        blockchain_source = jm_single().config.get(
    ...
    def on_JM_FIDELITY_BOND_PROOF_REQUEST(self, takernick, makernick):
        proof_msg = self.client.fidelity_bond.create_proof(
            makernick, takernick
        ).create_proof_msg(self.client.fidelity_bond.cert_privkey)
        d = self.callRemote(
            commands.JMFidelityBondProof, nick=takernick, proof=proof_msg
        )
        self.defaultCallbacks(d)
        return {"accepted": True}

    @commands.JMAuthReceived.responder
    def on_JM_AUTH_RECEIVED(
        self, nick, offer, commitment, revelation, amount, kphex
    ):
        retval = self.client.on_auth_received(
    ...
    def on_JM_TX_BROADCAST(self, tx):
        ...
        try:
            ...
            jlog.info(
                "We received an invalid transaction broadcast "
                "request: " + tx.hex()
            )
        return {"accepted": True}
    ...
    def unconfirm_callback(self, txd, txid):
        ...
        if not offerinfo:
            return False
        to_cancel, to_announce = self.client.on_tx_unconfirmed(offerinfo, txid)
        self.client.modify_orders(to_cancel, to_announce)

        txinfo = tuple((x.scriptPubKey, x.nValue) for x in txd.vout)
        confirm_timeout_sec = (
            float(jm_single().config.get("TIMEOUT", "confirm_timeout_hours"))
            * 3600
        )
        task.deferLater(
            reactor,
    ...
    def confirm_callback(self, txd, txid, confirms):
        ...
        to_cancel, to_announce = self.client.on_tx_confirmed(
            offerinfo, txid, confirms
        )
        self.client.modify_orders(to_cancel, to_announce)
        d = self.callRemote(
            commands.JMAnnounceOffers,
...
def start_reactor(  # noqa: C901
    ...
    if daemon:
        try:
            from jmdaemon import (
                ...
                BIP78ServerProtocolFactory,
            )
        except ImportError:
            jlog.error(
                "Cannot start daemon without jmdaemon package; "
        ...
        def start_daemon_on_port(p, f, name, port_offset):
            ...
            while True:
                try:
                    ...
                    jlog.info(
                        "{} daemon listening on port {}".format(
        ...
        if jm_coinjoin:
            ...
            jmcport, serverconn = start_daemon_on_port(
                port_a, dfactory, "Joinmarket", 0
            )
        ...
        if snickerfactory:
            snickerport, serverconn = start_daemon_on_port(
                port_a, sdfactory, "SNICKER", 1000
            )
    ...
    if usessl:
        ...
        if snickerfactory:
            reactor.connectSSL(
                host, snickerport, snickerfactory, ClientContextFactory()
            )
    else:
        if factory:
