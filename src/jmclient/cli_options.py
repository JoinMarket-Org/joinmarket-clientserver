...
def add_base_options(parser):
    ...
    parser.add_option(
        ...
        default=False,
        help=(
            'choose to do detailed wallet sync, '
...
def get_order_choose_algorithm(option, opt_str, value, parser, value_kw=None):
    ...
    if not fn:
        raise OptionValueError(
            "internal error: '{}' order choose algorithm not found".format(
                value
            )
        )
...
counterparties are selected."""
    ...
    if rel_prompt:
        ...
        def rel_check(val: str) -> bool:
            ...
            if val_float >= 1:
                print(
                    "Choose a number below 1! Else you will spend all your "
                    "bitcoins for fees!"
                )
                return False
            return True
...
def get_max_cj_fee_values(
    config, parser_options, user_callback=prompt_user_for_cj_fee
):
    ...
    return tuple(
        map(lambda j: fee_types[j](fee_values[j]), range(len(fee_values)))
    )


def check_regtest(blockchain_start=True):
    """Applies any regtest-specific configuration"""
    if not isinstance(jm_single().bc_interface, RegtestBitcoinCoreInterface):
        return
    if blockchain_start:
...
def get_tumbler_parser():
    ...
    parser.add_option(
        ...
        default=False,
        help=(
            'Restarts the schedule currently found in the schedule file in the '
    ...
    parser.add_option(
        ...
        help=(
            'Name of schedule file for tumbler, useful for restart, default '
            'TUMBLE.schedule'
        ),
    )
    parser.add_option(
        ...
        dest='addrcount',
        default=3,
        help='How many destination addresses in total should be used. If not enough are given'
    ...
    parser.add_option(
        ...
        help='Input the mean and spread of number of makers to use. e.g. 9 1 will be a normal distribution '
    ...
    parser.add_option(
        ...
        dest='minmakercount',
        default=4,
        help='The minimum maker count in a transaction, random values below this are clamped at this number. default=4',
    )
    parser.add_option(
        '-M',
        '--mixdepthcount',
    ...
    parser.add_option(
        ...
        default=3,
        help='Stage 1 sweep coinjoins have a longer wait time. This parameter'
    ...
    parser.add_option(
        ...
        help="If rounding happens (determined by --rounding-chance) then the weights of how many"
...
def get_sendpayment_parser():
    parser = OptionParser(
        usage='usage: %prog [options] wallet_file amount destination\n'
    ...
    parser.add_option(
        ...
        dest='pickorders',
        default=False,
        help='interactively pick which orders to take. doesn\'t work while sweeping.',
    )
    parser.add_option(
        '-m',
        '--mixdepth',
