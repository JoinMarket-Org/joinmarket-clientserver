...
class YieldGenerator(Maker):
    ...
    def __init__(self, wallet_service):
        ...
        self.statement_file = os.path.join(
            jm_single().datadir, 'logs', 'yigen-statement.csv'
        )
...
class YieldGeneratorBasic(YieldGenerator):
    ...
    def __init__(self, wallet_service, offerconfig):
        ...
        (
            self.txfee_contribution,
            self.cjfee_a,
            self.cjfee_r,
            self.ordertype,
            self.minsize,
            self.txfee_contribution_factor,
            self.cjfee_factor,
            self.size_factor,
        ) = offerconfig
    ...
    def create_my_orders(self):
        ...
        if self.ordertype in ('reloffer', 'swreloffer', 'sw0reloffer'):
            ...
            self.minsize = max(
                int(1.5 * self.txfee_contribution / float(self.cjfee_r)),
                self.minsize,
            )
        ...
        order = {
            ...
            - max(jm_single().DUST_THRESHOLD, self.txfee_contribution),
    ...
    def get_fidelity_bond_template(self):
        ...
        BLOCK_COUNT_SAFETY = (
            2  # use this safety number to reduce chances of the proof expiring
        )
        # before the taker gets a chance to verify it
        RETARGET_INTERVAL = 2016
        CERT_MAX_VALIDITY_TIME = 1
        cert_expiry = (
            (blocks + BLOCK_COUNT_SAFETY) // RETARGET_INTERVAL
        ...
        timelocked_utxos = [
            (outpoint, info)
            for outpoint, info in utxos.items()
        ...
        (utxo_priv, locktime), engine = (
            self.wallet_service.wallet._get_key_from_path(
    ...
    def oid_to_order(self, offer, amount):
        ...
        required_amount = (
            total_amount + jm_single().DUST_THRESHOLD + 1 - real_cjfee
        )

        mix_balance = self.get_available_mixdepths()
        filtered_mix_balance = {
            m: b for m, b in mix_balance.items() if b >= required_amount
        }
        if not filtered_mix_balance:
            return None, None, None
        jlog.debug(
            'mix depths that have enough = ' + str(filtered_mix_balance)
        )
        ...
        change_amount = (
            sum(u["value"] for u in utxos.values()) - total_amount + real_cjfee
        )
    ...
    def on_tx_confirmed(self, offer, txid, confirmations):
        if offer["cjaddr"] in self.tx_unconfirm_timestamp:
            confirm_time = (
                int(time.time()) - self.tx_unconfirm_timestamp[offer["cjaddr"]]
            )
        else:
            confirm_time = 0
        timestamp = datetime.datetime.now().strftime("%Y/%m/%d %H:%M:%S")
        real_cjfee = calc_cj_fee(
            offer["offer"]["ordertype"],
            offer["offer"]["cjfee"],
            offer["amount"],
        )
        self.log_statement(
            [
                ...
                offer["amount"],
                len(offer["utxos"]),
                sum([av['value'] for av in offer["utxos"].values()]),
                real_cjfee,
                real_cjfee - offer["offer"]["txfee"],
    ...
    def get_available_mixdepths(self):
        ...
        return self.wallet_service.get_balance_by_mixdepth(
    ...
    def select_change_address(
        self, input_mixdepth: int, change_amount: int
    ) -> str:
...
class YieldGeneratorService(Service):
    ...
    def startService(self):
        ...
        self.yieldgen = YieldGeneratorBasic(
            self.wallet_service, self.yg_config
        )
        self.clientfactory = JMClientProtocolFactory(
            self.yieldgen, proto_type="MAKER"
        )
        ...
        start_reactor(
            self.daemon_host, self.daemon_port, self.clientfactory, rs=False
        )
...
def ygmain(ygclass, nickserv_password='', gaplimit=6):
    ...
    if ordertype == 'reloffer':
        ...
        # of the miner fee
        minsize = max(
            int(1.2 * txfee_contribution / float(cjfee_r)),
            int(options["minsize"]),
        )
        cjfee_a = None
        ...
        parser.error(
            'You specified an incorrect offer type which '
    ...
    if jm_single().bc_interface is None:
        jlog.error(
            "Running yield generator requires configured "
            + "blockchain source."
        )
    ...
    if jm_single().config.get("SNICKER", "enabled") == "true":
        if jm_single().config.get("BLOCKCHAIN", "network") == "mainnet":
            jlog.error(
                "You have enabled SNICKER on mainnet, this is not "
