...
from jmclient import (
    ...
    BaseWallet,
    VolatileStorage,
    StoragePasswordError,
    is_segwit_mode,
    SegwitLegacyWallet,
    LegacyWallet,
    SegwitWallet,
    FidelityBondMixin,
    FidelityBondWatchonlyWallet,
...
"""
    parser = OptionParser(
        usage='usage: %prog [options] [wallet file] [method] [args..]',
    ...
    parser.add_option(
        ...
        type='int',
        dest='mixdepth',
        help="Mixdepth(s) to use in the wallet. Default: {}".format(
            DEFAULT_MIXDEPTH
        ),
        default=None,
    )
    ...
    parser.add_option(
        ...
        default=1,
        help=(
            'History method verbosity, 0 (least) to 6 (most), '
...
class WalletViewBase(object):
    ...
    def get_balance(self, include_unconf=True):
        if not include_unconf:
            raise NotImplementedError(
                "Separate conf/unconf balances not impl."
            )
    ...
    def get_balances(self, include_unconf=True):
        return (
            self.get_balance(include_unconf=include_unconf),
    ...
    def get_fmt_balance_json(self, include_unconf=True):
        ...
        return {
            self.balance_key_name: "{0:.08f}".format(total_balance),
...
class WalletViewEntry(WalletViewBase):
    ...
    def __init__(
        ...
        assert address_type in [
            SegwitWallet.BIP32_EXT_ID,
            SegwitWallet.BIP32_INT_ID,
            -1,
    ...
    def get_available_balance(self, include_unconf=True):
        return (
            0 if self.is_locked() or self.is_frozen() else self.get_balance()
        )
    ...
    def serialize_amounts(self, unconf_separate=False, denom="BTC"):
        if denom != "BTC":
            raise NotImplementedError(
                "Altern. denominations not yet implemented."
            )
        if unconf_separate:
            raise NotImplementedError(
                "Separate handling of unconfirmed funds not yet implemented."
            )
...
class WalletViewBranch(WalletViewBase):
    ...
    def serialize_branch_header(self):
        ...
        if self.address_type == -1:
            start = "Imported keys"
        return self.serclass(
            self.separator.join([start, self.wallet_path_repr, self.xpub])
        )
...
class WalletViewAccount(WalletViewBase):
    ...
    def __init__(
        ...
        self.xpub = xpub
        if branches:
            assert len(branches) in [
                2,
                3,
                4,
                5,
            ]  # 3 if imported keys, 4 if fidelity bonds
    ...
    def serialize(self, entryseparator="\n", summarize=False):
        ...
        if summarize:
            return self.serclass(
                entryseparator.join(
                    [x.serialize("", summarize=True) for x in self.branches]
                    + [footer]
                )
            )
        else:
            return self.serclass(
                entryseparator.join(
                    [header]
                    + [x.serialize(entryseparator) for x in self.branches]
                    + [footer]
                )
            )
    ...
    def serialize_json(self, summarize=False):
        ...
        if summarize:
            return json_serialized
        json_serialized["branches"] = [
            x.serialize_json() for x in self.branches
        ]
        return json_serialized
...
class WalletView(WalletViewBase):
    ...
    def serialize(self, entryseparator="\n", summarize=False):
        ...
        if summarize:
            return self.serclass(
                entryseparator.join(
                    [header]
                    + [x.serialize("", summarize=True) for x in self.accounts]
                    + [footer]
                )
            )
        else:
    ...
    def serialize_json(self, summarize=False):
        json_serialized = {
            ...
            "accounts": [
                x.serialize_json(summarize=summarize) for x in self.accounts
            ],
        }
        json_serialized.update(self.get_fmt_balance_json())
        return json_serialized
...
def get_tx_info(
    txid: bytes, tx_cache: Optional[dict] = None
) -> Tuple[bool, int, int, dict, int, btc.CTransaction]:
    """
    ...
    non_cj_freq = (
        0
        if len(value_freq_list) == 1
        else sum(next(islice(zip(*value_freq_list[1:]), 1, None)))
    )
...
def get_imported_privkey_branch(wallet_service, m, showprivkey):
    ...
    for data in wallet_service.get_utxos_at_mixdepth(
    ...
    for path in wallet_service.yield_imported_paths(m):
        ...
        entries.append(
            WalletViewEntry(
                wallet_service.get_path_repr(path),
                m,
                -1,
                0,
                addr,
...
def wallet_showutxos(
    ...
    utxos = wallet_service.get_utxos_by_mixdepth(
        ...
        for u, av in utxos[md].items():
            ...
            if locktime:
                unsp[us]["locktime"] = str(
                    datetime.utcfromtimestamp(0) + timedelta(seconds=locktime)
                )
    ...
    for u, ec in external_commitments.items():
        success, us = utxo_to_utxostr(u)
        assert success
        tries = podle.get_podle_tries(
            utxo=u, max_tries=max_tries, external=True
        )
        tries_remaining = max(0, max_tries - tries)
        unsp[us] = {
            'tries': tries,
...
def wallet_display(  # noqa: C901
    ...
    def get_addr_status(addr_path, utxos, utxos_enabled, is_new, is_internal):
        ...
        for utxo, utxodata in utxos.items():
            ...
            # to bci
            if not isinstance(
                jm_single().bc_interface, BitcoinCoreNoHistoryInterface
            ):
                is_coinjoin, cj_amount, cj_n = get_tx_info(utxo[0])[:3]
    ...
    for m in md_range:
        ...
        for address_type in [
            ...
            unused_index = wallet_service.get_next_unused_index(
                m, address_type
            )
            gap_addrs = []
            for k in range(unused_index + wallet_service.gap_limit):
                ...
                if (
                    displayall
                    or balance > 0
                    or (status == 'new' and address_type == 0)
                ):
                    entrylist.append(
                        WalletViewEntry(
                            wallet_service.get_path_repr(path),
            ...
            if (
                address_type == BaseWallet.ADDRESS_TYPE_EXTERNAL
                and wallet_service.bci is not None
            ):
            ...
            path = wallet_service.get_path_repr(
                wallet_service.get_path(m, address_type)
            )
            branchlist.append(
                WalletViewBranch(
                    path, m, address_type, entrylist, xpub=xpub_key
                )
            )

        if m == FidelityBondMixin.FIDELITY_BOND_MIXDEPTH and isinstance(
            ...
            for timenumber in range(FidelityBondMixin.TIMENUMBER_COUNT):
                ...
                addr = wallet_service.get_address_from_path(path)
                label = wallet_service.get_address_label(addr)
                timelock = datetime.utcfromtimestamp(0) + timedelta(
                    seconds=path[-1]
                )
                ...
                balance = sum(
                    [
                        utxodata["value"]
                        for _, utxodata in utxos[m].items()
                        if path == utxodata["path"]
                    ]
                )

                status = (
                    timelock.strftime("%Y-%m-%d")
                    + " ["
                    + ("LOCKED" if datetime.now() < timelock else "UNLOCKED")
                    + "]"
                )
                status += get_utxo_status_string(
                    utxos[m], utxos_enabled[m], path
                )
            ...
            wallet_service.set_next_index(
                m,
                address_type,
                unused_index + wallet_service.wallet.gap_limit,
                force=True,
            )
            for k in range(unused_index + wallet_service.wallet.gap_limit):
                ...
                if path_repr_b in burner_outputs:
                    txhex, blockheight, merkle_branch, blockindex = (
                        burner_outputs[path_repr_b]
                    )
                    ...
                    status = bintohex(txd.GetTxid()) + (
                        " [NO MERKLE PROOF]"
                        if merkle_branch
                        == FidelityBondMixin.MERKLE_BRANCH_UNAVAILABLE
                        else ""
                    )
                privkey = (
                    wallet_service.get_wif_path(path) if showprivkey else ""
                )
                if displayall or balance > 0:
                    entrylist.append(
                        WalletViewEntryBurnOutput(
                            path_repr,
                            m,
                            address_type,
                            k,
        ...
        acctlist.append(
            WalletViewAccount(path, m, branchlist, xpub=xpub_account)
        )
...
def cli_display_user_words(words: str, mnemonic_extension: str) -> None:
    ...
    if mnemonic_extension:
        text += (
            '\nAnd this mnemonic extension: '
            + mnemonic_extension.decode('utf-8')
            + '\n'
        )
...
def cli_do_use_mnemonic_extension() -> bool:
    if cli_prompt_user_yesno(
        "Would you like to use a two-factor mnemonic "
        "recovery phrase? "
...
def cli_get_mnemonic_extension() -> str:
    jmprint(
        "Note: This will be stored in a reversible way. Do not reuse!", "info"
    )
...
def cli_do_support_fidelity_bonds() -> bool:
    if cli_prompt_user_yesno(
        "Would you like this wallet to support "
        "fidelity bonds? "
        "Write 'n' if you don't know what this is"
    ):
        return True
...
def wallet_generate_recover_bip39(
    ...
    wallet_cls = get_wallet_cls(
        get_configured_wallet_type(support_fidelity_bonds)
    )
...
def wallet_fetch_history(wallet, options):  # noqa: C901
    ...
    while len(buf) == 1000:
        ...
        # confirmed
        tx_data = (
            (tx['txid'], tx['blockhash'], tx['blocktime'], 0)
            for tx in buf
            if 'txid' in tx and 'blockhash' in tx and 'blocktime' in tx
        )
        tx_db.executemany(
            'INSERT INTO transactions VALUES(?, ?, ?, ?);', tx_data
        )
        # unconfirmed
        uc_tx_data = (
            (tx['txid'], None, None, len(tx['walletconflicts']))
            for tx in buf
            if 'txid' in tx and 'blockhash' not in tx and 'blocktme' not in tx
        )
        tx_db.executemany(
            'INSERT INTO transactions VALUES(?, ?, ?, ?);', uc_tx_data
        )
    txes = tx_db.execute(
    ...
    wallet_script_set = set(
        wallet.get_script_from_path(p) for p in wallet.yield_known_paths()
    )
    ...
    def print_row(
        ...
        data = [
            index,
            datetime.fromtimestamp(time).strftime("%Y-%m-%d %H:%M"),
            tx_type,
            btc.sat_to_str(abs(amount)),
    ...
    if options.verbosity > 0:
        jmprint(s().join(field_names), "info")
    ...
    for tx in txes:
        ...
        if len(our_input_scripts) == 0 and len(our_output_scripts) > 0:
            ...
            len(our_input_scripts) > 0
            and len(our_output_scripts) == 2
            and is_coinjoin
        ):
            out_value = sum(
            ...
            cj_script = list(
                set(
                    [
                        a
                        for a, v in output_script_values.items()
                        if v == cj_amount
                    ]
                ).intersection(our_output_scripts)
            )[0]
            mixdepth_dst = wallet.get_script_mixdepth(cj_script)
        elif len(our_input_scripts) > 0 and len(our_output_scripts) == len(
            output_script_values
        ):
            out_value = sum(
                [output_script_values[a] for a in our_output_scripts]
    ...
    if abs(total_profit) > 0:
        try:
            ...
            def f(r, deposits, deposit_times, now, final_balance):
                return (
                    np.sum(
                        np.exp((now - deposit_times) / 60.0 / 60 / 24 / 365)
                        ** r
                        * deposits
                    )
                    - final_balance
            ...
            r = brentq(
                f, a=1, b=-1, args=(deposits, deposit_times, now, balance)
            )
            jmprint(
            ...
            jmprint(
                'to add it to this virtual environment, use `pip install scipy`'
            )

    # includes disabled utxos in accounting:
    total_wallet_balance = sum(
        wallet.get_balance_by_mixdepth(include_disabled=True).values()
    )
    if balance + unconfirmed_balance != total_wallet_balance:
        jmprint(
            ...
            % (btc.sat_to_str(total_wallet_balance), btc.sat_to_str(balance))
        )
    wallet_utxo_count = sum(
        map(len, wallet.get_utxos_by_mixdepth(include_disabled=True).values())
    )
    if utxo_count + unconfirmed_utxo_count != wallet_utxo_count:
        jmprint(
            ...
            % (wallet_utxo_count, utxo_count)
        )

    if unconfirmed_balance != 0:
        jmprint(
            'unconfirmed balance change = %s BTC'
            % btc.sat_to_str(unconfirmed_balance)
        )
...
def wallet_importprivkey(wallet, mixdepth):
    ...
    jmprint(
        "WARNING: Make sure that the type of the public address previously "
    ...
    if import_failed:
        jmprint(
            "Warning: failed to import {} keys".format(import_failed), "error"
        )
...
def wallet_signmessage(
    wallet, hdpath: str, message: str, out_str: bool = True
...
def wallet_signpsbt(wallet_service, psbt):
    ...
    if signresult.is_final:
        if not cli_prompt_user_yesno(
            "Above PSBT is fully signed. Do you want to broadcast?"
        ):
            ...
            if jm_single().bc_interface.pushtx(tx.serialize()):
                jmprint("Transaction sent: " + bintohex(tx.GetTxid()[::-1]))
            else:
        ...
        jmprint(
            "The PSBT is not yet fully signed, we signed: {} inputs.".format(
                signresult.num_inputs_signed
            )
        )
    return ""


def display_utxos_for_disable_choice_default(
    ...
    def default_user_choice(umax):
        jmprint(
            "Choose an index 0 .. {} to freeze/unfreeze or "
        ...
        while True:
            ...
            if ret < -2 or ret > umax:
                jmprint(
                    "Invalid choice, must be between: -2 and {}, "
                    "try again.".format(umax),
                    "error",
                )
                continue
            break
...
def get_utxos_enabled_disabled(
    wallet_service: WalletService, md: int
) -> Tuple[dict, dict]:
...
def wallet_freezeutxo(
    wallet_service, md, display_callback=None, info_callback=None
):
    ...
    while True:
        ...
        if utxos_disabled == {} and utxos_enabled == {}:
            info_callback(
                "The mixdepth: "
                + str(md)
                + " contains no utxos to freeze/unfreeze.",
                "error",
            )
            return "Failed"
        ...
        if display_ret == "all":
            ...
            info_callback(
                "Setting all UTXOs to "
                + ("frozen" if disable else "unfrozen")
                + " . . ."
            )
            ...
            if disable:
                info_callback(
                    "Utxo: {} is now frozen and unavailable for spending.".format(
                        fmt_utxo((txid, index))
                    )
                )
            else:
                info_callback(
                    "Utxo: {} is now unfrozen and available for spending.".format(
                        fmt_utxo((txid, index))
                    )
                )
    return "Done"
...
def wallet_gettimelockaddress(wallet, locktime_string):
    ...
    if (
        jm_single().config.get("BLOCKCHAIN", "network") == "mainnet"
        and lock_datetime <= datetime.now()
    ):
    ...
    jmprint(
        "WARNING: Only send coins here which are from coinjoins or otherwise"
...
def wallet_addtxoutproof(wallet_service, hdpath, txoutproof):
    ...
    new_merkle_branch = jm_single().bc_interface.core_proof_to_merkle_branch(
        txoutproof
    )
    txhex = binascii.hexlify(txhex).decode()
    txid = btc.txhash(txhex)
    if not jm_single().bc_interface.verify_tx_merkle_branch(
        txid, block_height, new_merkle_branch
    ):
        jmprint("Error: tx out proof invalid", "error")
        return ""
    wallet_service.wallet.add_burner_output(
        hdpath, txhex, block_height, new_merkle_branch, block_index
    )
    return "Done"
...
def wallet_createwatchonly(wallet_root_path, master_pub_key):
    ...
    wallet = create_wallet(
        ...
        wallet_cls=FidelityBondWatchonlyWallet,
        entropy=entropy,
    )
    return "Done"
...
def get_configured_wallet_type(support_fidelity_bonds):
    ...
    if configured_type == TYPE_P2WPKH:
        ...
        raise ValueError(
            "Fidelity bonds not supported with the configured "
...
def get_wallet_cls(wtype):
    ...
    if not cls:
        raise WalletError(
            "No wallet implementation found for type {}.".format(wtype)
        )
    return cls
...
def create_wallet(path, password, max_mixdepth, wallet_cls, **kwargs):
    ...
    wallet_cls.initialize(
        storage, get_network(), max_mixdepth=max_mixdepth, **kwargs
    )
    storage.save()
    return wallet_cls(
        storage, gap_limit=jm_single().config.getint("POLICY", "gaplimit")
    )


def open_test_wallet_maybe(
    path,
    seed,
    max_mixdepth,
    ...
    if len(seed) == test_wallet_cls.ENTROPY_BYTES * 2:
        try:
            ...
            test_wallet_cls.initialize(
                storage, get_network(), max_mixdepth=max_mixdepth, entropy=seed
            )
...
def open_wallet(
    path, ask_for_password=True, password=None, read_only=False, **kwargs
):
    """
    ...
    if not Storage.is_storage_file(path):
        raise Exception(
            "Failed to open wallet at '{}': not a valid joinmarket"
            " wallet.".format(path)
        )
    ...
    if ask_for_password and Storage.is_encrypted_storage_file(path):
        while True:
            try:
                ...
                pwd = (
                    get_password("Enter passphrase to decrypt wallet: ")
                    or None
                )
                ...
                jmprint(
                    "Failed to load wallet, error message: " + repr(e), "error"
                )
                raise e
            break
    else:
...
def get_wallet_cls_from_storage(storage):
    ...
    if wtype is None:
        raise WalletError(
            "File {} is not a valid wallet.".format(storage.path)
        )
...
def wallet_sanity_check(wallet):
    if wallet.network != get_network():
        raise Exception(
            "Wallet network mismatch: we are on '{}' but wallet "
...
def wallet_tool_main(wallet_root_path):  # noqa: C901
    ...
    if args[0] in noseed_methods:
        ...
        if (
            method not in noscan_methods
            and jm_single().bc_interface is not None
        ):
    ...
    if method == "display":
        return wallet_display(
            wallet_service, options.showprivkey, mixdepth=options.mixdepth
        )
        ...
        if len(args) < 3:
            jmprint(
                'Must have locktime value yyyy-mm. For example 2021-03',
                "error",
            )
            sys.exit(EXIT_ARGERROR)
