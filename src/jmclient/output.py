...
def fmt_tx_data(tx_data, wallet_service):
    return 'path: {}, address: {} , value: {}'.format(
        wallet_service.get_path_repr(
            wallet_service.script_to_path(tx_data['script'])
        ),
        wallet_service.script_to_addr(tx_data['script']),
        tx_data['value'],
    )
...
def generate_podle_error_string(
    ...
    errmsg = ""
    errmsgheader = (
        "Failed to source a commitment; this debugging information"
        " may help:\n\n"
    )
    errmsg += (
        "1: Utxos that passed age and size limits, but have "
    ...
    if len(priv_utxo_pairs) == 0:
        ...
        for p, u in priv_utxo_pairs:
            errmsg += fmt_utxo(u) + "\n"
    errmsg += (
        "2: Utxos that have less than " + taker_utxo_age + " confirmations:\n"
    )
    if len(to) == 0:
        errmsg += "None\n"
    else:
        for t in to:
            errmsg += fmt_utxo(t) + "\n"
    errmsg += (
        "3: Utxos that were not at least "
        + taker_utxo_amtpercent
        + "% of the size of the coinjoin amount "
        + str(cjamount)
        + "\n"
    )
    if len(ts) == 0:
    ...
    errmsg += (
        "Utxos only in item 3 are not big enough for this "
