...
from jmclient.support import (
    calc_cj_fee,
    fidelity_bond_weighted_order_choose,
    choose_orders,
    choose_sweep_orders,
)
from jmclient.wallet import (
    estimate_tx_fee,
    compute_tx_locktime,
    FidelityBondMixin,
...
class Taker(object):
    ...
    def set_honest_only(self, truefalse):
        ...
        if truefalse:
            if not len(self.honest_makers):
                jlog.debug(
                    "Attempt to set honest-only without "
    ...
    def initialize(self, orderbook, fidelity_bonds_info):
        ...
        if self.schedule_index == len(self.schedule):
            ...
            if self.my_cj_addr == "INTERNAL":
                ...
                jlog.info(
                    "Choosing a destination from mixdepth: "
                    + str(next_mixdepth)
                )
        ...
        fidelity_bond_values = calculate_fidelity_bond_values(
            fidelity_bonds_info
        )
    ...
    def prepare_my_bitcoin_data(self):
        ...
        if self.cjamount != 0:
            if self.custom_change_address:
                ...
                try:
                    self.my_change_addr = (
                        self.wallet_service.get_internal_addr(self.mixdepth)
                    )
            ...
            ) * (self.n_counterparties - 1) + estimate_tx_fee(
                3,
                2,
                txtype=self.wallet_service.get_txtype(),
                outtype=self.wallet_service.get_outtype(
                    self.coinjoin_address()
                ),
            )
            total_amount = self.cjamount + self.total_cj_fee + self.total_txfee
            jlog.info(
                'total estimated amount spent = '
    ...
    def receive_utxos(self, ioauth_data):
        ...
        for maker_inputs in verified_data:
            ...
            self.outputs.append(
                {
                    'address': maker_inputs.change_addr,
                    'value': maker_inputs.change_amount,
                }
            )
            self.outputs.append(
                {'address': maker_inputs.cj_addr, 'value': self.cjamount}
            )
            self.cjfee_total += maker_inputs.real_cjfee
            self.maker_txfee_contributions += self.orderbook[
                maker_inputs.nick
            ]['txfee']
        ...
        if self.my_change_addr is None:
            ...
            self.outputs.append(
                {'address': self.my_change_addr, 'value': my_change_value}
            )
        self.utxo_tx = [u for u in sum(self.utxos.values(), [])]
        self.outputs.append(
            {'address': self.coinjoin_address(), 'value': self.cjamount}
        )
        ...
        self.latest_tx = btc.make_shuffled_tx(
            self.utxo_tx, self.outputs, version=n_version, locktime=locktime
        )
        jlog.info(
        ...
        return (
            True,
            list(self.maker_utxo_data.keys()),
    ...
    def _verify_ioauth_data(self, ioauth_data):
        ...
        for nick, nickdata in ioauth_data.items():
            ...
            if (
                not validate_address(cj_addr)[0]
                or not validate_address(change_addr)[0]
            ):
                jlog.warn(
                    "Counterparty provided invalid address: {}".format(
                        (cj_addr, change_addr)
                    )
                )
    ...
    def _verify_ioauth_inputs(self, nick, utxo_list, auth_pub):
        ...
        return self._MakerTxData(
            nick, utxo_data, total_input, change_amount, real_cjfee
        )
    ...
    def auth_counterparty(self, btc_sig, auth_pub, maker_pk):
        ...
        try:
            ...
            jlog.info(
                "Failed ecdsa verify for maker pubkey: " + bintohex(maker_pk)
            )
            jlog.info("Exception was: " + repr(e))
            return False
        return True

    def on_sig(self, nick, sigb64):  # noqa: C901
        ...
        if nick not in self.nonrespondants:
            jlog.debug(
                ('add_signature => nick={} not in nonrespondants {}').format(
        ...
        for i, u in utxo.items():
            ...
            is_witness_input = (
                scriptPubKey.is_p2sh() or scriptPubKey.is_witness_v0_keyhash()
            )
            ver_amt = utxo_data[i]['value'] if is_witness_input else None
            witness = (
                btc.CScriptWitness([ver_sig, ver_pub])
                if is_witness_input
                else None
            )
            ...
            if sig_good:
                ...
                if len(self.utxos[nick]) == 0:
                    jlog.debug(
                        (
                            'nick = {} sent all sigs, removing from '
                            'nonrespondant list'
                        ).format(nick)
                    )
                    try:
        ...
        tx_signed = True
        for ins, witness in zip(
            self.latest_tx.vin, self.latest_tx.wit.vtxinwit
        ):
            if (
                ins.scriptSig == b""
        ...
        jlog.debug(
            "schedule item was: " + str(self.schedule[self.schedule_index])
        )
    ...
    def make_commitment(self):
        ...
        def filter_by_coin_age_amt(utxos, age, amt):
            results = jm_single().bc_interface.query_utxo_set(
                utxos, includeconfs=True
            )
            newresults = []
            too_new = []
            too_small = []
        ...
        def priv_utxo_pairs_from_utxos(utxos, age, amt):
            ...
            priv_utxo_pairs = []
            new_utxos, too_new, too_small = filter_by_coin_age_amt(
        ...
        if podle_data:
            ...
            return (
                commit_type_byte + bintohex(podle_data.commitment),
            ...
            with open("commitments_debug.txt", "wb") as f:
                errmsgfileheader = (
                    b"THIS IS A TEMPORARY FILE FOR DEBUGGING; "
    ...
    def handle_unbroadcast_transaction(self, txid, tx):
        ...
        if not self.wallet_service.check_callback_called(
            ...
            if jm_single().config.get('POLICY', 'tx_broadcast') == "not-self":
                warnmsg = (
                    "You have chosen not to broadcast from your own "
    ...
    def push(self):
        ...
        if tx_broadcast == 'self':
            ...
            jlog.info(
                "Only self, random-peer and not-self broadcast "
    ...
    def confirm_callback(self, txd, txid, confirmations):
        ...
        fromtx = (
            False if self.schedule_index + 1 == len(self.schedule) else True
        )
...
def round_to_significant_figures(d, sf):
    ...
    for p in range(-10, 15):
        ...
        if power10 > d:
            sf_power10 = 10**sf
            sigfiged = int(
                round(d / power10 * sf_power10) * power10 / sf_power10
            )
            return sigfiged
    raise RuntimeError()
...
def calculate_fidelity_bond_values(fidelity_bonds_info):
    ...
    for bond_data in fidelity_bonds_info:
        ...
        if fb_proof.utxo in validated_bonds:
            continue
        utxo_data = (
            FidelityBondMixin.get_validated_timelocked_fidelity_bond_utxo(
                fb_proof.utxo,
                fb_proof.utxo_pub,
                fb_proof.locktime,
                fb_proof.cert_expiry,
                blocks,
            )
        )
    ...
    fidelity_bond_values = {
        bond_data.maker_nick: FidelityBondMixin.calculate_timelocked_fidelity_bond_value(
            utxo_data["value"],
