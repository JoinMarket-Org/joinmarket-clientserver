...
class BlockchainInterface(ABC):
    ...
    @abstractmethod
    def import_addresses_if_needed(
        self, addresses: Set[str], wallet_name: str
    ) -> bool:
    ...
    def import_addresses(
        self,
        addr_list: Iterable[str],
        wallet_name: str,
        restart_cb: Optional[Callable[[str], None]] = None,
    ) -> None:
    ...
    @abstractmethod
    def get_deser_from_gettransaction(
        self, rpcretval: dict
    ) -> Optional[btc.CMutableTransaction]:
    ...
    @abstractmethod
    def get_tx_merkle_branch(
        self, txid: str, blockhash: Optional[str] = None
    ) -> bytes:
        """TODO: describe method."""

    @abstractmethod
    def verify_tx_merkle_branch(
        self, txid: str, block_height: int, merkle_branch: bytes
    ) -> bool:
        """TODO: describe method."""
    ...
    @abstractmethod
    def _estimate_fee_basic(
        self, conf_target: int
    ...
    def yield_transactions(self) -> Generator[dict, None, None]:
        ...
        while True:
            ...
            try:
                idx = [
                    (tx['txid'], tx['vout'], tx['category']) for tx in txs
                ].index(
                    (
                        yielded_tx['txid'],
                        yielded_tx['vout'],
    ...
    def get_unspent_indices(self, transaction: btc.CTransaction) -> List[int]:
        ...
        res = self.query_utxo_set(
            [(bintxid, i) for i in range(len(transaction.vout))]
        )
    ...
    def estimate_fee_per_kb(self, tx_fees: int) -> int:
        ...
        tx_fees_factor = abs(
            jm_single().config.getfloat('POLICY', 'tx_fees_factor')
        )
        ...
        if self.fee_per_kb_has_been_manually_set(tx_fees):
            N_res = random.uniform(
                tx_fees, tx_fees * float(1 + tx_fees_factor)
            )
        ...
        if feerate < mempoolminfee_in_sat:
            ...
            msg = (
                "Using bitcoin network feerate for "
                + str(tx_fees)
...
class BitcoinCoreInterface(BlockchainInterface):
    def __init__(
        self, jsonRpc: JsonRpc, network: str, wallet_name: str
    ) -> None:
        super().__init__()
        ...
        if wallet_name:
            ...
            self.descriptors = (
                "descriptors" in wallet_info and wallet_info["descriptors"]
            )
    ...
    def rescanblockchain(
        ...
        Thread(
            target=self._rescan_in_thread, args=(start_height,), daemon=True
        ).start()
    ...
    def import_addresses(
        ...
        addr_list: Iterable[str],
        wallet_name: str,
        restart_cb: Callable[[str], None] = None,
    ) -> None:
        requests = []
        if self.descriptors:
        ...
        if num_failed > 0:
            fatal_msg = (
                "Fatal sync error: import of {} address(es) failed for "
    ...
    def import_addresses_if_needed(
        ...
        if wallet_name in self._rpc('listlabels', []):
            imported_addresses = set(
                self._rpc('getaddressesbylabel', [wallet_name]).keys()
            )
        else:
            imported_addresses = set()
    ...
    def pushtx(self, txbin: bytes) -> bool:
        ...
        try:
            ...
            log.warning(
                'error pushing = ' + str(e.code) + " " + str(e.message)
            )
            return False
        return True
    ...
    def get_current_block_height(self) -> int:
        try:
            ...
            raise RuntimeError(
                "Getblockcount RPC failed with: %i, %s" % (e.code, e.message)
            )
    ...
    def verify_tx_merkle_branch(
        ...
        core_proof = (
            self._rpc("getblockheader", [block_hash, False])
...
class BitcoinCoreNoHistoryInterface(
    BitcoinCoreInterface, RegtestBitcoinCoreMixin
):
    def __init__(
        self, jsonRpc: JsonRpc, network: str, wallet_name: str
    ) -> None:
    ...
    def import_addresses_if_needed(
        ...
        if self.import_addresses_call_count == 1:
            ...
            self.scan_result = self._rpc("scantxoutset", ["start", addr_list])
            et = time.time()
    ...
    def listaddressgroupings(self) -> list:
        raise RuntimeError(
            "default sync not supported by bitcoin-rpc-nohistory, use --recoversync"
        )
...
class RegtestBitcoinCoreInterface(
    ...
    def estimate_fee_per_kb(self, tx_fees: int) -> int:
        if not self.absurd_fees:
            ...
            return (
                jm_single().config.getint("POLICY", "absurd_fee_per_kb") + 100
            )
    ...
    def pushtx(self, txbin: bytes) -> bool:
        if (
            self.pushtx_failure_prob != 0
            and random.random() < self.pushtx_failure_prob
        ):
            log.debug(
                'randomly not broadcasting %0.1f%% of the time'
        ...
        if not self.simulating and self.tick_forward_chain_interval > 0:
            log.debug(
                'will call tfc after '
                + str(self.tick_forward_chain_interval)
                + ' seconds.'
            )
            reactor.callLater(
                self.tick_forward_chain_interval, self.tick_forward_chain, 1
            )
        return ret
            ...
            res.append(
                {
                    ...
                    'balance': int(
                        Decimal(1e8)
                        * self._rpc('getreceivedbyaddress', [address, 0])
                    ),
                }
            )
        return {'data': res}
